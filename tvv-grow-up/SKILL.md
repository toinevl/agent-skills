...
category: meta
scope: [all]
triggers: [something breaks, mistake happens, pattern fails, prevent recurrence]
status: stable
created: '2026-07-22'
...
Most problems aren't one-time accidents—they're symptoms of broken systems. The "tvv-grow-up" skill turns every problem into a permanent improvement: analyze what went wrong, fix the root cause, and update the tools/processes/skills so it can't happen the same way again.
...
2. **Commit with good message:**
   ```
   tvv-grow-up: learn from [problem] + prevent recurrence
...
1. **Created skill: `tvv-roomsense-wishlist-first`**
...
3. **Committed:**
   ```
   tvv-grow-up: learn from wishlist divergence + prevent recurrence
   
   Root cause: Mental model treated docs as outputs, not work items.
   No pre-flight discipline or enforcement.
   
   Fix:
   - Created skill: tvv-roomsense-wishlist-first (checklist + guidance)
...
✅ Fix created: New skill "tvv-roomsense-wishlist-first" with pre-work checklist

✅ Rule updated: Added "Wishlist-First Discipline" to CLAUDE.md

✅ Committed: tvv-grow-up commit explaining the learning
...
**The tvv-grow-up principle:** Every mistake is a teacher. Extract the lesson, automate the prevention, commit the knowledge, and move forward stronger.
